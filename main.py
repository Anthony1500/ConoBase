...
class CustomItemDelegate(QStyledItemDelegate):
    def sizeHint(self, option, index):
        ...
        size.setHeight(size.height() + 10)  # Margen inferior de 5 píxeles entre filas
        return size
...
# Establecer el ícono en el método paint
        icon_path = "images/flecha_right.png"
        self.icon = QtGui.QIcon(icon_path)
        super(CustomItemDelegate, self).paint(painter, option, index)
        rect = option.rect
        iconSize = QtCore.QSize(16, 16)
        iconRect = QtCore.QRect(int(rect.right() - iconSize.width() - 5),
                        int(rect.top() + (rect.height() - iconSize.height()) / 2),
       ...
       # Dibuja el ícono
        ...
        self.icon.paint(painter, iconRect, QtCore.Qt.AlignCenter)
