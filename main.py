   ...
   def clicked_button_guardar(self):
       ...
       if not text.strip():
        ...
        # Crear la ventana emergente aquí
            ...
            dialog_text.setText("Error: Por favor, asegúrate de ingresar algún texto.")
        ...
        if text not in nombres:
           ...
           with open("resources/"+"cat_"+text+".txt", "w") as archivo:
...
class ListCat(QtWidgets.QDialog):
    def __init__(self, stacked_widget,my_object2):
        super().__init__()
        ...
        self.lista_cat.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        ...
        myDelegate = CustomItemDelegate(stacked_widget,my_object2,my_object3)  # Crear una instancia de         
...
class ListsubCat(QtWidgets.QDialog):
    def __init__(self, stacked_widget,my_object2,my_object3):
        ...
        self.installEventFilter(self)
        self.lista_subcat.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
    ...
    def button_clicked_nuevo(self): 
        ...
        self.stacked_widget.setCurrentIndex(5)  # Cambiar a la página 0 del QStackedWidget
    ...
    def actualizar_lista(self):
        ...
        myDelegate = CustomItemDelegatesubcat(my_stacked_widget,my_object2)  # Crear una instancia de         
        ...
        self.timer.timeout.connect(self.update_data)  # Conectar la señal timeout a la función update_data
    ...
    def button_clicked(self):        
        ...
        self.inputtext_search_subcat.clear()
        self.stacked_widget.setCurrentIndex(2)  # Cambiar a la página 0 del QStackedWidget
    ...
    def update_data(self):           
        ...
        if datos != '':
            ...
            file_name = 'resources/cat_'+datos+'.txt'            
...
class CustomItemDelegate(QStyledItemDelegate):    
    ...
    def editorEvent(self, event, model, option, index):
       ...
       if event.type() == QEvent.MouseButtonRelease:
            ...
            if respuesta == QMessageBox.Yes:
                ...
                dialog_text = form.dialog_text  # Acceder al  formulario
                dialog_text.setWordWrap(True)  
                dialog_text.setText("¡Éxito! La categoría seleccionada ha sido eliminada correctamente.")
                ...
                self.stacked_widget.setCurrentIndex(4)  # Cambiar a la página 4 del QStackedWidget
          ...
          if event.type() == QEvent.MouseButtonPress:
            ...
            if event.pos().x() < item_center.x():
                ...
                # ...
                    ...
                    with open("resources/cat_"+data+".txt", 'r') as file:
    ...
    def sizeHint(self, option, index):
        ...
        size.setHeight(size.height() + 9) # Margen inferior de 10 píxeles entre filas
    ...
    def paint(self, painter, option, index):
        ...
        iconRect1 = QtCore.QRect(int(rect.right() - iconSize.width()),
                        int(rect.top() + (rect.height() - iconSize.height()) / 2), #background 
        ...
        self.button_rect_trash = QtCore.QRect(int(rect.right() - button_size_trash.width()- 79),
                        int(rect.top() + (rect.height() - button_size_trash.height()) / 2), #button trash
        ...
        self.button_rect_right = QtCore.QRect(int(rect.right() - button_size_right.width()- 4),
                        int(rect.top() + (rect.height() - button_size_right.height()) / 2),  #button arrow right
        ...
        if option.state & QStyle.State_Selected:
          ...
          selection_rect.setWidth(226 if len(nombres) > 6 else 240)  # Limitar la anchura de la selección
          ...
          selection_rect.setWidth(226 if len(nombres) > 6 else 240)  # Limitar la anchura de la selección
          painter.fillRect(selection_rect, brush)  # Color de fondo personalizado
    ...
    def editorEvent(self, event, model, option, index):
       ...
       if event.type() == QEvent.MouseButtonRelease:
            if self.button_rect_trash.contains(event.pos()):     
               ...
               # Mostrar un cuadro de diálogo preguntando al usuario si desea eliminar el archivo
               respuesta = QMessageBox.question(self.parent(), "Eliminar archivo", f"¿Estás seguro de que deseas eliminar esta entrada: <b>{nombre_archivo}</b>?")
            
            # Si el usuario acepta, eliminar el archivo
            if respuesta == QMessageBox.Yes:
                # Construir la ruta del archivo
                text= self.my_object3.get_text().split('\n')[0] 
                file_name = "resources/cat_"+text+".txt"
                ...
                dialog_text = form.dialog_text  # Acceder al  formulario
                dialog_text.setWordWrap(True)  
                dialog_text.setText("¡Éxito! La entrada seleccionada ha sido eliminada correctamente.")
          ...
          if event.type() == QEvent.MouseButtonPress:
            ...
            if event.pos().x() < item_center.x():
                ...
                # ...
                    ...
                    #contenido
                    file_name = 'resources/cat_'+text_ca+'.txt'
                    title = data
                    date = datahour
...
class ComtenCat(QtWidgets.QDialog):
    ...
    def button_clicked_guardar(self):  
        ...
        if not text.strip() and not text1.strip():
           ...
           dialog_text = form.dialog_text  # Acceder al  formulario
           dialog_text.setWordWrap(True) 
           dialog_text.setText("Error: Por favor, asegúrate de ingresar algún texto en los campos.")
           ...
           self.panel.show()
            ...
            if text == my_object.get_text():
             ...
             dialog_text = form.dialog_text  # Acceder al  formulario
             dialog_text.setWordWrap(True) 
             dialog_text.setText("Error: Por favor, asegúrate de ingresar algún texto diferente a la categoría.")
           ...
           #  Colocar el diálogo en la parte superior de la ventana principal
             ...
             self.panel.show()
                ...
                with open("resources/" +"cat_"+ my_object.get_text() + ".txt", "a") as archivo:       
    ...
    def button_clicked_cancelar(self):        
        ...
        # Construir la ruta del archivo
        directorio = 'resources'
        ruta_archivo = os.path.join(directorio, "cat_"+my_object.get_text()+".txt")                
        # Eliminar el archivo
        os.remove(ruta_archivo)
...
class ComtensubCat(QtWidgets.QDialog):
        ...
        if not text.strip() and not text1.strip():
           ...
           #  Colocar el diálogo en la parte superior de la ventana principal
             ...
             self.panel.show()
                ...
                file_name = 'resources/cat_'+my_object3.get_text().split('\n')[0]+'.txt'
                title = my_object3.get_text().split('\n')[0]
                date = my_object3.get_text().split('\n')[1]
                self.add_box(file_name, title, date)
                
                
                self.stacked_widget.setCurrentIndex(4)  # Cambiar a la página 2 del QStackedWidget
                self.inputtext_create_cat_2.clear()   
                self.inputtext_create_cat_3.clear() 
    ...
    def button_clicked_cancelar(self):    
        self.inputtext_create_cat_2.clear()   
        self.inputtext_create_cat_3.clear() 
        self.stacked_widget.setCurrentIndex(4)  # Cambiar a la página 0 del QStackedWidget
...
if __name__ == "__main__":
    ...
    search_cat_window = ListCat(stacked_widget,my_object2)
